<template>
  <div class="mobile-list" :class="{ remote: remote }" :style="{ height }">
    ...
    <!-- 上下滚动主体 -->
    <div style="position: relative; height: 100%; overflow: hidden;" ref="full-wrapper">
      <!-- 左右滚动主体 -->
...
export default {
  ...
  mounted() {
    if (!this.fullScroll) {
      this.fullScroll = new BScroll(this.$refs['full-wrapper'], {
        click: true,
        scrollY: true,
        bounce: false,
        probeType: 3 // 监听滑动需要设置probeType 1 非实时 3 实时
      });
      this.fullScroll.scrollTo(0, 0);
    } else {
      this.fullScroll.refresh();
    }
    if (!this.scroll) {
    ...
    if (this.headFixed) {
      this.scroll.on("scroll", pos => {
        this.$refs.topFixedTable.style.transform = `translateX(${pos.x}px)`;
        if (pos.x !== 0) {
